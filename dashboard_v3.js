    ...
    try {
        ...
        if (specialList) {
            specialList.innerHTML = data.campaigns
                .filter(c => ['membership', 'multipass', 'certificates'].includes(c.type))
                .map(c => `
                <div class="metric-card rounded-[2rem]" style="cursor:pointer; background: #FFFFFF; border: 1px solid #F1F5F9; border-radius: 32px; box-shadow: 0 8px 30px rgba(0,0,0,0.04);" onclick="openCampaignHub('${c.id}')">
...
window.updateUnifiedDesc = function(val) {
    ...
    window.showToast = function(message, type = "info") {
        ...
        if (!container) {
            ...
            style.innerHTML = `
                .toast-container { position: fixed; bottom: 40px; left: 50%; transform: translateX(-50%); display: flex; flex-direction: column; gap: 12px; z-index: 999999; pointer-events: none; align-items: center; }
...
// 1. LEADS (PROSPECTOS)
    ...
    try {
        ...
        if (state && state.restaurantName && currentEmail.trim().toLowerCase() !== 'hola@fideliorewards.com') {
                ...
                document.getElementById('header-business-category').innerHTML = `<span>${bCatDisp}</span>`;
