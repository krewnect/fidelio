...
window.renderScheduleDays = function() {
    ...
    days.forEach(day => {
            ...
            shifts.forEach((shift, index) => {
                shiftsHtml += `
                    <div style="display:flex; align-items:center; gap:12px; margin-bottom: ${index === shifts.length - 1 ? '0' : '12px'};">
                        <input type="time" class="premium-input schedule-time-input" data-day="${day}" data-index="${index}" data-type="start" value="${shift.start}" style="padding:10px 14px; font-size:14px; font-family:inherit; border-radius:10px; background:#1f2937; border:1px solid rgba(255,255,255,0.2); width:130px; color:var(--text-main);">
                        <span style="color:var(--text-muted); font-size:13px; font-weight:600;">a</span>
                        <input type="time" class="premium-input schedule-time-input" data-day="${day}" data-index="${index}" data-type="end" value="${shift.end}" style="padding:10px 14px; font-size:14px; font-family:inherit; border-radius:10px; background:#1f2937; border:1px solid rgba(255,255,255,0.2); width:130px; color:var(--text-main);">
        ...
        const dayHtml = `
            <div style="display:flex; justify-content:space-between; align-items:flex-start; padding:20px 32px; border-bottom:1px solid rgba(255,255,255,0.05); background: ${shifts.length === 0 ? 'rgba(255,255,255,0.02)' : 'transparent'}; transition: background 0.2s ease;">
