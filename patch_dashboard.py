...
# 1. Update the row rendering inside loadMerchantsControl
            ...
            tbody.innerHTML += `
                <tr style="border-bottom: 1px solid #F3F4F6; transition: background 0.2s;" onmouseover="this.style.background='#F9FAFB'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="padding: 16px; text-align: right;">
                        ...
                        <button class="fidelio-btn-secondary-preset" onclick="changeMerchantPlan('${m.id}', 'active')" style="padding:6px 12px; font-size:12px;"><i class="fa-solid fa-check"></i> Activar</button>
                    </td>
                </tr>
            `;"""
...
if old_row in js:
    ...
    print("WARNING: Could not find old_row pattern. Let's try appending directly.")

# Append logic to the very bottom
new_functions = """
...
# Because replacing failed, let's just do a regex replace on the specific row rendering inside loadMerchantsControl
# Specifically we want to replace the buttons in the row:
js = re.sub(
    r'<button class="fidelio-btn-secondary-preset" onclick="changeMerchantPlan\([^)]+\)[^>]+><i class="fa-solid fa-check"></i> Activar</button>',
    r'<button class="fidelio-btn-secondary-preset" onclick="openAdminMerchant(\'${m.id}\')" style="background:var(--bg-main); color:var(--text-main); padding:6px 12px; font-size:12px; border:1px solid var(--border-soft);"><i class="fa-solid fa-sliders"></i> Administrar</button>',
    js
)
js = re.sub(
    r'<button class="fidelio-btn-secondary-preset" onclick="grantFreeAccount\([^)]+\)[^>]+><i class="fa-solid fa-gift"></i></button>',
    r'',
    js
)
js = re.sub(
    r'<button class="fidelio-btn-secondary-preset" onclick="contactMerchant\([^)]+\)[^>]+><i class="fa-solid fa-envelope"></i> Aviso</button>',
    r'<button class="fidelio-btn-secondary-preset" onclick="openAdminMerchant(\'${m.id}\')"><i class="fa-solid fa-gear"></i> Administrar</button>',
    js
)

# Append to the very end but before the last }); if it exists
if js.strip().endswith('});'):
    js = js[:js.rfind('});')] + new_functions
else:
    js = js + new_functions.replace('\n});', '')

with open('dashboard_v2.js', 'w', encoding='utf-8') as f:
    f.write(js)
print("JS logic updated via regex and appended.")
